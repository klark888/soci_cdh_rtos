...
static uint16_t req_crc_table[] = {0x0000, 0x1021, 0x2042, 0x3063, 0x4084, 0x50a5, 0x60c6, 0x70e7,
...
static uint16_t rpl_crc_table[] = {0x0000, 0x1021, 0x2042, 0x3063, 0x4084, 0x50a5, 0x60c6, 0x70e7,
        0x8108, 0x9129, 0xa14a, 0xb16b, 0xc18c, 0xd1ad, 0xe1ce, 0xf1ef,
        0x1231, 0x0210, 0x3273, 0x2252, 0x52b5, 0x4294, 0x72f7, 0x62d6,
        0x9339, 0x8318, 0xb37b, 0xa35a, 0xd3bd, 0xc39c, 0xf3ff, 0xe3de,
        0x2462, 0x3443, 0x0420, 0x1401, 0x64e6, 0x74c7, 0x44a4, 0x5485,
...
void SPI_GPIO_init() {
    ...
    GPIO_PinInit( GPIO1, 16, &pcs_config );
    GPIO_PinInit( GPIO1, 17, &pcs_config );
    GPIO_PinInit( GPIO1, 18, &pcs_config );
    GPIO_PinInit( GPIO1, 19, &pcs_config );
}
  ...
  *req_packet_len_pt = 0;
  ...
  req_len_A = *req_payload_len_pt;
        ...
        for (ii = 0; ii < req_len_A; ii++) {
          crc_value = ((crc_value << 8) ^ req_crc_table[((crc_value >> 8) ^ req_array_A[ii]) & 0x00FF]);
        }
...
void rplPacketProcess(uint8_t *rpl_payload_pt, uint8_t *rpl_packet_pt, uint8_t *rpl_payload_len_pt, uint8_t *rpl_packet_len_pt) { //(NEEDS WORK) --- --- --- --- --- --- --- --- ---
  ...
  for (ii = 0; ii < rpl_len_W; ii++) {
    crc_value = ((crc_value << 8) ^ rpl_crc_table[((crc_value >> 8) ^ rpl_array_W[ii]) & 0x00FF]);
  }
